...
// Apply stock filter
    if (filterType === 'low-stock') {
      filtered = filtered.filter(item => item.quantity <= (item.low_stock_threshold || item.lowStockThreshold));
...
const getStockStatus = (item) => {
    const threshold = item.low_stock_threshold || item.lowStockThreshold || 0;
    if (item.quantity <= threshold) {
      return { status: 'low', color: 'error', label: 'Low Stock' };
} else if (item.quantity <= threshold * 1.5) {
...
const getFilterCount = (filterId) => {
    switch (filterId) {
      ...
      case 'low-stock':
        return inventory.filter(item => item.quantity <= (item.low_stock_threshold || item.lowStockThreshold)).length;
      case 'in-stock':
        return inventory.filter(item => item.quantity > (item.low_stock_threshold || item.lowStockThreshold)).length;
      default:
        return 0;
                        ...
                        <motion.div
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-center">
                                ...
                                <div className="md:col-span-2 flex items-center space-x-3">
                                    ...
                                    <div>
                                        <h3 className="font-medium text-gray-900">{item.Name || item.name}</h3>
                                        <p className="text-sm text-gray-500">Updated: {formatLastUpdated(item.last_updated || item.lastUpdated)}
                                        </p>
                                    </div>
                                </div>
                                {/* Current Stock */}
                                <div>
                                    ...
                                    <p className="text-xs text-gray-400">Min: {item.low_stock_threshold || item.lowStockThreshold} {item.unit}
                                    </p>
                                </div>
                                {/* Status */}
                                <div>
