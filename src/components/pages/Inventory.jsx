        ...
        {loading ? (
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            <div className="p-6 space-y-4">
              {Array.from({ length: 8 }).map((_, index) => (
                <div key={index} className="animate-pulse flex items-center justify-between py-4 border-b border-gray-100 last:border-0">
          ...
          >
            ...
            <div className="bg-gray-50 px-6 py-4 border-b border-gray-200">
              <div className="grid grid-cols-1 md:grid-cols-5 gap-4 text-sm font-medium text-gray-600">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-center">
                      ...
                      <div className="md:col-span-2 flex items-center space-x-3">
                        <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                          stockInfo.status === 'low' ? 'bg-error/10' :
                          stockInfo.status === 'medium' ? 'bg-warning/10' : 'bg-success/10'
                        }`}>
                          <ApperIcon 
                            name={getStockIcon(stockInfo.status)} 
                            className={`w-5 h-5 ${
                              stockInfo.status === 'low' ? 'text-error' :
                              stockInfo.status === 'medium' ? 'text-warning' : 'text-success'
                            }`} 
                          />
                        </div>
                        <div>
